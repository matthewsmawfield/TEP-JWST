#!/usr/bin/env python3
"""
Step 88: Big Bang Nucleosynthesis (BBN) Compatibility Analysis
...
Output:
- results/outputs/step_88_bbn_analysis.json
...
# TEP parameters
alpha_0 = 0.58  # Coupling constant from Cepheids
rho_c_screening = 20  # g/cm^3 (critical screening density)
...
def hubble_tep(z, alpha=alpha_0):
    ...
    # The effective coupling scales as (matter fraction)
    matter_fraction = Omega_m * (1 + z)**3 / (Omega_r * (1 + z)**4 + Omega_m * (1 + z)**3)
    ...
    return H0 * np.sqrt(G_eff_ratio) * np.sqrt(
...
def compute_deuterium_shift(delta_H_mean):
    ...
    d(D/H)/(D/H) ≈ -1.6 * dH/H (faster expansion = less D destruction)
    """
    DH_standard = 2.5e-5
    sensitivity = -1.6
...
def main():
    ...
    print("=" * 60)
    print("Step 88: BBN Compatibility Analysis")
    print("=" * 60)
    ...
    output_file = output_dir / 'step_88_bbn_analysis.json'
