#!/usr/bin/env python3
"""
TEP-JWST Step 34: CEERS Data Download and Processing
...
def download_ceers_catalog():
    ...
    if CEERS_CATALOG_FILE.exists():
        size_mb = CEERS_CATALOG_FILE.stat().st_size / 1e6
        if size_mb > 100:  # Valid file should be ~194 MB
            print_status(f"CEERS catalog already exists: {CEERS_CATALOG_FILE} ({size_mb:.1f} MB)", "INFO")
            return True
        else:
            print_status(f"Existing file too small ({size_mb:.1f} MB), re-downloading...", "INFO")
            CEERS_CATALOG_FILE.unlink()
    
    print_status(f"Downloading CEERS DR1 catalog from:", "INFO")
    print_status(f"  {CEERS_CATALOG_URL}", "INFO")
    print_status("This may take a few minutes (~194 MB)...", "INFO")
    
    try:
        urllib.request.urlretrieve(CEERS_CATALOG_URL, CEERS_CATALOG_FILE)
        size_mb = CEERS_CATALOG_FILE.stat().st_size / 1e6
        print_status(f"Downloaded: {size_mb:.1f} MB", "INFO")
        return True
