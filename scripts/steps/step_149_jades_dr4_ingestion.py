...
"""
TEP-JWST Step 149: JADES DR4 Spectroscopic Catalog Ingestion
...
Outputs:
- results/interim/step_149_jades_dr4_specz.csv
- results/outputs/step_149_jades_dr4_ingestion.json
...
def download_jades_dr4():
    """Download JADES DR4 spectroscopic catalog if not already present."""
    return smart_download(
        url=JADES_DR4_URL,
        dest=JADES_DR4_FILE,
        min_size_mb=JADES_DR4_SIZE_MB_MIN,
        logger=logger,
    )


def load_jades_dr4():
    """Download (if needed) and load JADES DR4 spectroscopic catalog."""
    if not JADES_DR4_FILE.exists() or JADES_DR4_FILE.stat().st_size / 1e6 < JADES_DR4_SIZE_MB_MIN:
        if not download_jades_dr4():
            print_status("ERROR: Could not obtain JADES DR4 catalog. Aborting.", "ERROR")
            return None, None

    print_status(f"Loading JADES DR4: {JADES_DR4_FILE}", "PROCESS")

    with fits.open(JADES_DR4_FILE) as hdul:
...
def analyze_tep_signal(df):
    ...
    if len(z7) >= 5:
        ...
        results['z_gt_7'] = {
            ...
            'spearman_p': max(float(p7), 1e-300),
    ...
    if len(z8) >= 5:
        ...
        results['z_gt_8'] = {
            ...
            'spearman_p': max(float(p8), 1e-300),
...
def cross_survey_check(df_jades):
    ...
    if len(jades_valid) >= 10:
        ...
        results['jades_gamma_t_vs_MUV'] = {
            'rho': float(rho_j), 'p': max(float(p_j), 1e-300), 'N': len(jades_valid)
        }
    ...
    if 'gamma_t' in df_uncover.columns and 'dust' in df_uncover.columns:
        ...
        if len(unc_valid) >= 10:
            ...
            results['uncover_gamma_t_vs_dust'] = {
                'rho': float(rho_u), 'p': max(float(p_u), 1e-300), 'N': len(unc_valid)
            }
...
def main():
    ...
    print_status(f"STEP {STEP_NUM}: JADES DR4 Spectroscopic Catalog Ingestion", "INFO")
    print_status("=" * 60, "INFO")
    ...
    # Save interim CSV
    out_csv = INTERIM_PATH / f"step_{STEP_NUM}_jades_dr4_specz.csv"
