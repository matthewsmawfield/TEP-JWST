...
from scripts.utils.tep_model import compute_gamma_t as tep_gamma, ALPHA_0, Z_REF, LOG_MH_REF
...
def solve_scalar_profile_absolute(halo_mass_Msun=1e12, concentration=10, beta=1.0):
    """
    Computes the ABSOLUTE clock rate Γ = A(φ) × √(1 + 2Φ_N).
    ...
    """
    logger.info(f"Solving ABSOLUTE profiles for M_h={halo_mass_Msun:.1e}, c={concentration}, β={beta}")
    ...
    # Absolute clock rate: Γ = A × √(1 + 2Φ_N)
...
def run_analysis():
    ...
    results = {
        ...
        'explanation': 'TEP uses RELATIVE enhancement Γ_t compared to a reference environment, '
                       'not ABSOLUTE clock rates. This naturally produces Γ_t > 1 for M_h > M_ref '
    ...
    # =========================================================================
    # PART 1: Show why the ABSOLUTE formulation has issues at low β
    # =========================================================================
    print_status("\n--- Part 1: Absolute Clock Rate Analysis ---", "INFO")
    ...
    # 1. The ABSOLUTE formulation requires 4β² > 1 (β > 0.5) for enhancement
    ...
    # 
    # In the absolute formulation with β, for the same potential depth:
    # Γ = exp(-4β² Φ_N) × √(1 + 2Φ_N)
    ...
    theoretical_mapping = {
        ...
        'sign_paradox_resolution': 'The sign paradox arises from conflating ABSOLUTE clock rates '
        ...
        'physical_meaning': 'Γ_t > 1 means the galaxy experiences MORE effective time than the reference. '
                            'This is a statement about differential evolution, not absolute clock rates.'
    }
    ...
    # Save results
    with open(OUTPUT_PATH / "step_90_sign_paradox.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print_status(f"\nResults saved to {OUTPUT_PATH / 'step_90_sign_paradox.json'}", "INFO")
    
    # =========================================================================
    # PART 5: Generate Figures
    # =========================================================================
    
    # Figure 1: Absolute vs Relative comparison
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    
    # Left: Absolute clock rate for various β
    ax1 = axes[0]
    ...
    ax1.set_ylabel('Absolute Clock Rate Γ = A×√(1+2Φ)')
    ax1.set_title('Absolute Formulation\n(Shows sign paradox for β < 0.5)')
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_PATH / "step_90_sign_paradox_resolution.png", dpi=150)
    plt.close()
    
    print_status(f"Figure saved to {FIGURES_PATH / 'step_90_sign_paradox_resolution.png'}", "INFO")
