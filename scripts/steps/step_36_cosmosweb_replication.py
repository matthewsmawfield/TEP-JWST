#!/usr/bin/env python3
"""
TEP-JWST Step 36: COSMOS-Web Independent Replication
...
Requires: step_35_cosmosweb_download.py to be run first.
...
STEP_NUM = "36"
STEP_NAME = "cosmosweb_replication"
...
def load_cosmosweb_data():
    ...
    if not cosmosweb_file.exists():
        ...
        print_status("Run step_35_cosmosweb_download.py first.", "INFO")
        return None
...
def test_gamma_dust_correlation(df, sample_name):
    ...
    slope_g, int_g, _, _, _ = linregress(df_valid['log_Mstar'], df_valid['gamma_t'])
    gamma_resid = df_valid['gamma_t'] - (slope_g * df_valid['log_Mstar'] + int_g)
    
    slope_d, int_d, _, _, _ = linregress(df_valid['log_Mstar'], df_valid['dust'])
    dust_resid = df_valid['dust'] - (slope_d * df_valid['log_Mstar'] + int_d)
