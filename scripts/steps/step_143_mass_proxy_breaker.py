...
def load_data():
    ...
    path = INTERIM_PATH / "step_002_uncover_full_sample_tep.csv"
    if not path.exists():
        path = INTERIM_PATH / "step_001_uncover_full_sample.csv"
    if not path.exists():
...
def main():
    logger.info("=" * 70)
    logger.info("Step 143: Mass-Proxy Degeneracy Breaker")
    logger.info("=" * 70)
    ...
    results = {
        "step": "Step 143: Mass-Proxy Degeneracy Breaker",
        "description": (
