...
def load_json(path):
    if not path.exists():
        print_status(f"WARNING: {path} not found. Using default values.", "WARNING")
        return {}
    try:
        with open(path) as f:
            return json.load(f)
    except Exception as e:
        print_status(f"WARNING: Could not load {path}: {e}", "WARNING")
        return {}
...
def main():
    ...
    "interpretation": {
        "main_finding": (
            f"The mass-matched step-function M* model (AIC={aic4:.1f}) outperforms the "
            f"step-function t_eff model (AIC={aic3:.1f}) by ΔAIC={delta_aic_step:.1f}, "
            f"despite identical parameter counts (k=2). The best-fitting model overall is "
            f"the polynomial M*×z baseline (AIC={aic5:.1f}, Akaike weight=1.0). "
            f"Both step-function models are dominated by the polynomial baseline "
            f"(ΔAIC={delta_aics['step_teff_agb']:.1f} and {delta_aics['step_mass_matched']:.1f} respectively)."
        ),
        "ols_limitation": (
            "OLS linear regression penalizes t_eff because the AGB threshold creates a "
            "non-linear (step-function) relationship. However, even with the step-function "
            "functional form, the mass-matched threshold achieves lower AIC. The dust ratio "
            f"above/below the t_eff threshold is only {simulated_dust_ratio:.2f}×, indicating "
            "a weak step contrast in the UNCOVER z>8 sample."
        ),
        ...
        "conclusion": (
            f"The step-function t_eff model does not outperform the mass-matched step-function "
            f"M* model on AIC (ΔAIC={delta_aic_step:.1f}) or BIC (ΔBIC={delta_bic_step:.1f}). "
            f"The polynomial M*×z model dominates all competitors. The AGB step-function "
            f"contrast (dust ratio {simulated_dust_ratio:.2f}×) is present but weak in the "
            f"UNCOVER z>8 sample. The case for t_eff over M* rests on the rank-correlation "
            f"and Steiger Z tests (step_136), not on the step-function AIC comparison."
        ),
    },
}
    ...
    print_status("Step 141 complete.", "SUCCESS")
    print_status(f"Best AIC model: {result['key_comparisons']['best_model_aic']}", "INFO")
    print_status(f"Step t_eff vs step M* ΔAIC: {result['key_comparisons']['step_teff_vs_step_mass_delta_aic']}", "INFO")
    print_status(f"Step t_eff wins AIC: {result['key_comparisons']['step_teff_wins_aic']}", "INFO")
    print_status(f"Step t_eff wins BIC: {result['key_comparisons']['step_teff_wins_bic']}", "INFO")
    print_status(f"Dust ratio above/below t_eff threshold: {simulated_dust_ratio:.2f}x", "INFO")
    print_status(f"Output: {out_path}", "INFO")
