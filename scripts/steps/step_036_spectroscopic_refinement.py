#!/usr/bin/env python3
# Estimated runtime from last full canonical run (2026-03-09 15:52 UTC; full pipeline 32m18s): 1.0s.
"""
...
def load_data():
    ...
    if not path.exists():
        generated = ensure_combined_spectroscopic_catalog(path)
        if generated is not None:
            print_status(f"Built spectroscopic catalog from local reproducible inputs: {generated.name}", "INFO")
    if not path.exists():
        import logging
        logging.getLogger(__name__).error(
            f"Missing: {path.name} — run spectroscopic ingestion steps first.")
        return None
    df = pd.read_csv(path)
...
def main():
    ...
    df = load_data()
    if df is None:
        print_status("combined_spectroscopic_catalog.csv not found. Aborting.", "ERROR")
    ...
    z46 = results['bin_analysis'].get('z4_6')
    if z46 and z46['p'] < 0.05 and results['simpsons_check']['rho_norm'] > 0:
