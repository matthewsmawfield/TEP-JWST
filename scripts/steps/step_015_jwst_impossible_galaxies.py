...
STEP_NUM = "015"  # Pipeline step number
STEP_NAME = "jwst_impossible_galaxies"  # Used in log / output filenames
...
def identify_impossible_galaxies(df):
    """Identify "anomalous" galaxies with age_ratio > threshold.

    The age_ratio = t_stellar / t_cosmic measures what fraction of the
    available cosmic time has elapsed since the galaxy's stellar populations
    formed. Under standard physics:
      - age_ratio < 0.5: normal (formed in the second half of cosmic time)
      - age_ratio > 0.5: problematic (implies very early formation)
      - age_ratio > 1.0: physically impossible (stellar age > cosmic age)

    At z > 6, t_cosmic < 1 Gyr, so even modest SED-fitted ages push
    age_ratio above 0.5. These are the galaxies that JWST has been
    reporting as "impossibly mature" or "too old too early".

    Under TEP, high age_ratio is expected for high-Gamma_t systems:
    the enhanced proper time makes stellar populations evolve faster,
    producing SED fits that infer older ages than the cosmic time allows.
    """
    logger.info("=" * 70)
...
def compare_gamma_t(anomalous, normal):
    """Compare mean Gamma_t between anomalous and normal galaxies.

    TEP prediction:
      Galaxies classified as "anomalous" (age_ratio > 0.5) should have
      systematically higher Gamma_t than "normal" galaxies, because the
      enhanced proper time in deeper potentials is what causes the
      apparent age excess.

    Statistics:
      - Welch's t-test for mean difference
      - Cohen's d for effect size (d > 0.5 = medium, d > 0.8 = large)
      - Ratio of means (anomalous / normal) for intuitive comparison
    """
    logger.info("=" * 70)
...
def analyze_age_ratio_gamma_correlation(df):
    """Test direct correlation between age_ratio and Gamma_t.

    This is the most direct TEP test in this step: higher Gamma_t
    should predict higher age_ratio because the enhanced proper time
    causes SED fitting to infer older stellar ages.

    Under standard physics with no TEP, age_ratio and Gamma_t
    should not correlate after accounting for the trivial mass-age
    and mass-halo_mass relationships. A significant positive Spearman
    rho constitutes evidence for TEP.
    """
    logger.info("=" * 70)
