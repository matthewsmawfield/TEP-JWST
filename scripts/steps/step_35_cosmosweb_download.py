#!/usr/bin/env python3
"""
TEP-JWST Step 35: COSMOS-Web Data Download and Processing
...
def download_cosmosweb_catalog():
    ...
    # Check if file exists and is valid size (~257 MB)
    if COSMOSWEB_LEPHARE_FILE.exists():
        size_mb = COSMOSWEB_LEPHARE_FILE.stat().st_size / 1e6
        if size_mb > 200:
            print_status(f"COSMOS-Web LePhare catalog already exists: {COSMOSWEB_LEPHARE_FILE.name} ({size_mb:.1f} MB)", "INFO")
            return True
        else:
            print_status(f"Existing file too small ({size_mb:.1f} MB), re-downloading...", "INFO")
            COSMOSWEB_LEPHARE_FILE.unlink()
    
    # Get credentials from environment (do not hardcode credentials)
    ...
    url = "https://cosmos2025.iap.fr/data/catalog/COSMOSWeb_mastercatalog_v1_lephare.fits"
    
    print_status(f"Downloading COSMOS-Web LePhare catalog...", "INFO")
    print_status(f"This may take a few minutes (~257 MB)...", "INFO")
    print_status("", "INFO")
    
    try:
        # Use curl with verbose progress
        result = subprocess.run([
            'curl', '-u', f'{username}:{password}',
            '-L', '--progress-bar',
            '-o', str(COSMOSWEB_LEPHARE_FILE),
            url
        ], check=True)
        
        size_mb = COSMOSWEB_LEPHARE_FILE.stat().st_size / 1e6
        print_status(f"\nDownloaded: {size_mb:.1f} MB", "INFO")
        return True
    except subprocess.CalledProcessError as e:
        print_status(f"Download failed: {e}", "INFO")
        print_status("\nTo download manually:", "INFO")
        print_status("  1. Visit https://cosmos2025.iap.fr/catalog_download.php", "INFO")
        print_status("  2. Download 'LePhare Redshifts, Physical Parameters'", "INFO")
        print_status(f"  3. Save to: {COSMOSWEB_LEPHARE_FILE}", "INFO")
        return False
...
def main():
    ...
    print_status("STEP 35: COSMOS-Web Data Download and Processing", "INFO")
    print_status("=" * 70, "INFO")
    print_status("", "INFO")
