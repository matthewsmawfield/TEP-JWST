...
"""
TEP-JWST Step 10: SN Ia Host Galaxy Mass Step Analysis
...
def analyze_continuous_correlation(df):
    ...
    results = {
        "spearman_rho": rho,
        "spearman_p": format_p_value(p_spearman),
        "pearson_r": r,
        "pearson_p": format_p_value(p_pearson),
        "linear_slope": slope,
...
def binned_mass_analysis(df, n_bins=5):
    ...
    return {
        ...
        "bin_correlation_p": format_p_value(p_bins)
    }
...
def run_sn_ia_mass_step_analysis():
    ...
    # Save results
    output_file = RESULTS_DIR / f"step_{STEP_NUM}_sn_ia_mass_step.json"
