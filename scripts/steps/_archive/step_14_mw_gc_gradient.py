#!/usr/bin/env python3
"""
TEP-JWST Step 14: Milky Way Globular Cluster Age-Galactocentric Distance Test

This is the "critical signal" gradient test for TEP. If TEP affects stellar ages,
...
The TEP prediction:
    Age_observed = Age_true * (1 + Gamma_t)
...
def load_gc_ages():
    ...
    Primary sources:
    - VandenBerg et al. (2013): 55 GCs with absolute ages
    - Marín-Franch et al. (2009): 64 GCs with relative ages
    - Kruijssen et al. (2019): 96 GCs compiled from multiple sources
    
    We use the mean ages from Kruijssen et al. (2019) which combines
    multiple studies.
    """
...
def analyze_age_rgc_correlation(df):
    ...
    Standard physics prediction: No correlation (GCs formed at similar times)
    TEP prediction: NEGATIVE correlation (inner GCs appear older)
    """
    logger.info("=" * 70)
...
def run_mw_gc_gradient_analysis():
    ...
    logger.info("=" * 70)
    logger.info("TEP-JWST Step 14: MW Globular Cluster Age-Galactocentric Distance Test")
    logger.info("=" * 70)
    logger.info("")
    logger.info("The 'critical signal' gradient test for TEP:")
    ...
    logger.info(f"Age-R_gc correlation: ρ = {corr['spearman_rho']:.3f} (p = {corr['spearman_p']:.4f})")
    ...
    if abs(corr['spearman_rho']) < 0.15 and corr['spearman_p'] > 0.1:
    ...
    # Save results
    output_file = RESULTS_DIR / "mw_gc_gradient_analysis.json"
