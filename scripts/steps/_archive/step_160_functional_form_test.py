#!/usr/bin/env python3
"""
Step 160: Functional Form Discrimination Test
...
Outputs:
- results/outputs/step_160_functional_form_test.json
...
def steiger_z_test(rho_jk, rho_jh, rho_kh, n):
    ...
    # Determinant of 3x3 correlation matrix
    r_bar = (rho_jk + rho_jh) / 2.0
    det = (1 - rho_jk**2 - rho_jh**2 - rho_kh**2
           + 2 * rho_jk * rho_jh * rho_kh)

    # Meng, Rosenthal & Rubin (1992) variance formula
    f = (1 - rho_kh) / (2 * max(1 - r_bar**2, 1e-10))
    h = 1 + f * (1 - rho_kh)

    se = np.sqrt(2 * h / max(n - 3, 1))
    z_stat = (z_jk - z_jh) / max(se, 1e-10)
...
def analyze_subsample(df, label):
    ...
    for pname, pvals in predictors.items():
        ...
        corrs[pname] = {"rho": float(rho), "p": float(p)}
...
def make_figure(results, output_path):
    ...
    fig.suptitle("Step 160: Functional Form Discrimination Test", fontsize=14, fontweight="bold")
...
def main():
    logger.section("Step 160: Functional Form Discrimination Test")
    ...
    results = {
        "step": "Step 160: Functional Form Discrimination Test",
        "total_n": len(df),
