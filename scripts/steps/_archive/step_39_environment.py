...
def load_catalog():
    ...
    path = DATA_INTERIM / "uncover_highz_sed_properties.csv"
    if not path.exists():
    ...
    if 'gamma_t' not in df.columns:
        ...
        if 'log_Mstar' in df.columns and 'z' in df.columns:
            ...
            alpha_z = ALPHA_0 * np.sqrt(1 + df['z'])
            z_factor = (1 + df['z']) / (1 + Z_REF)
            delta_log_Mh = log_Mh - LOG_MH_REF
            argument = alpha_z * (2/3) * delta_log_Mh * z_factor
            df['gamma_t'] = np.exp(argument)
