...
class DustEvolutionModel:
    ...
    def run_evolution(self):
        ...
        for i in range(1, len(t)):
            ...
            # Fraction disrupted = M_cleared * SN_Rate / M_gas
...
def main():
    ...
    # Count "Anomalous" (Deficit > 2x to account for uncertainties)
    impossible_std = len(res_df[res_df['deficit_std'] > 2])
    impossible_tep = len(res_df[res_df['deficit_tep'] > 2])
    
    print(f"Anomalous in Standard: {impossible_std}/{len(res_df)}")
    print(f"Anomalous in TEP: {impossible_tep}/{len(res_df)}")
