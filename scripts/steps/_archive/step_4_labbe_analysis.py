"""
TEP-JWST Step 4: Labbé+23 "Anomalous Galaxy" Analysis
Analyzes the original Labbé et al. 2023 sample of massive high-z galaxies
to test the TEP Chronological Shear prediction.

These galaxies are "anomalous" because their high stellar masses require
...
def estimate_stellar_age_from_mass(log_Mstar, z):
    ...
    This is what makes these galaxies "anomalous" - the required
...
def load_labbe_2023():
    """
    Load the Labbé et al. 2023 "anomalous galaxy" sample from their
...
def fit_mass_age_relation(df):
    ...
    # Use only galaxies with positive age excess (the "anomalous" ones)
...
def run_labbe_analysis():
    ...
    """
    logger.info("=" * 60)
    logger.info("TEP-JWST: Labbé+23 'Anomalous Galaxy' Analysis")
    logger.info("=" * 60)
    ...
    logger.info(f"'Anomalous' galaxies: {fit_results['n_impossible']}/{len(df)}")
