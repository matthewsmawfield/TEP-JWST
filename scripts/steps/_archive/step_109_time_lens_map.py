#!/usr/bin/env python3
"""
Step 109: Time-Lens Map (Effective Redshift) Analysis
...
Outputs:
- results/outputs/step_109_time_lens_map.json
...
def load_survey_data():
    ...
    uncover_path = INTERIM_PATH / "step_02_uncover_full_sample_tep.csv"
...
def make_figure(df_all, out_path: Path):
    try:
        ...
        for survey, g in df_all.groupby('survey'):
            ...
            ax_tc.scatter(g['t_cosmic_gyr'], g['dust'], s=12, alpha=0.5, color=c, label=survey)
            ax_te.scatter(g['t_eff_gyr'], g['dust'], s=12, alpha=0.5, color=c)
            ax_zo.scatter(g['z_obs'], g['dust'], s=12, alpha=0.5, color=c)
            ax_ze.scatter(g['z_eff'], g['dust'], s=12, alpha=0.5, color=c)

        ax_tc.set_xlabel('t_cosmic (Gyr)')
        ax_tc.set_ylabel('Dust')
        ax_tc.set_title('Dust vs t_cosmic')

        ax_te.set_xlabel('t_eff (Gyr)')
        ax_te.set_ylabel('Dust')
        ax_te.set_title('Dust vs t_eff')

        ax_zo.set_xlabel('z_obs')
        ax_zo.set_ylabel('Dust')
        ax_zo.set_title('Dust vs z_obs')

        ax_ze.set_xlabel('z_eff')
        ax_ze.set_ylabel('Dust')
        ax_ze.set_title('Dust vs z_eff')

        ax_tc.legend(frameon=False, loc='best')
...
def main():
    ...
    results = {
        ...
        'sources': {
            'UNCOVER': str(INTERIM_PATH / 'step_02_uncover_full_sample_tep.csv'),
    ...
    out_json = OUTPUT_PATH / 'step_109_time_lens_map.json'
