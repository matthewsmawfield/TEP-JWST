#!/usr/bin/env python3
"""
TEP-JWST Step 15: Extended SN Ia Analysis
...
TEP_H0_DATA = Path("/Users/matthewsmawfield/www/TEP-H0/data/raw/Pantheon+SH0ES.dat")
...
def analyze_mass_step_redshift_evolution(df):
    ...
    return {
        ...
        'spearman_rho': rho,
        'spearman_p': p_value,
        'evolution_detected': rho > 0 and p_value < 0.1
    }
...
def analyze_residual_correlations(df):
    ...
    for col, name in properties:
        if col in valid.columns:
            ...
            correlations[col] = {'name': name, 'rho': rho, 'p': p}
...
def run_extended_sn_analysis():
    ...
    logger.info("=" * 70)
    logger.info("TEP-JWST Step 15: Extended SN Ia Analysis")
    logger.info("=" * 70)
    logger.info("")
    ...
    # Save results
    output_file = RESULTS_DIR / "sn_ia_extended_analysis.json"
