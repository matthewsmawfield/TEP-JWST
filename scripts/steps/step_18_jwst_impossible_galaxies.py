#!/usr/bin/env python3
"""
TEP-JWST Step 18: "Impossible" Galaxies Analysis

The most dramatic TEP signature in JWST data is the existence of "impossible"
galaxies—systems whose inferred stellar ages exceed the cosmic age at their
redshift. Under standard physics, this is paradoxical. Under TEP, it's expected.

This analysis:
1. Identifies galaxies with age ratio > 0.5 (stellar age / cosmic age)
2. Tests if these "impossible" galaxies have systematically higher Γ_t
3. Quantifies the TEP explanation for the impossibility

The key prediction: galaxies that appear "impossible" should have the highest
predicted Γ_t values. This is a direct, quantitative test of TEP.

Author: Matthew L. Smawfield
Date: January 2026
"""

import numpy as np
import pandas as pd
from scipy import stats
from pathlib import Path
import logging
import json

# Setup paths
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_DIR = PROJECT_ROOT / "data"
RESULTS_DIR = PROJECT_ROOT / "results" / "outputs"

# Setup logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)


def load_uncover_data():
    """Load UNCOVER data with TEP calculations."""
    logger.info("Loading UNCOVER data with TEP parameters...")
    
    df = pd.read_csv(DATA_DIR / "interim" / "uncover_highz_sed_properties.csv")
    
    # Calculate age ratio
    df['age_ratio'] = df['mwa_Gyr'] / df['t_cosmic_Gyr']
    
    logger.info(f"Loaded {len(df)} galaxies")
    logger.info(f"Redshift range: {df['z'].min():.1f} - {df['z'].max():.1f}")
    logger.info(f"Age ratio range: {df['age_ratio'].min():.3f} - {df['age_ratio'].max():.3f}")
    
    return df


def identify_impossible_galaxies(df):
    """
    Identify "impossible" galaxies with age ratio > threshold.
    
    Standard physics: age_ratio should be < 1 (stellar age < cosmic age)
    In practice, age_ratio > 0.5 is already problematic (galaxies formed
    in the first half of cosmic time at that redshift).
    """
    logger.info("=" * 70)
    logger.info("ANALYSIS 1: Identifying 'Impossible' Galaxies")
    logger.info("=" * 70)
    
    thresholds = [0.3, 0.5, 0.7, 0.9]
    
    results = {}
    for thresh in thresholds:
        n_impossible = (df['age_ratio'] > thresh).sum()
        frac = n_impossible / len(df) * 100
        logger.info(f"Age ratio > {thresh}: N = {n_impossible} ({frac:.1f}%)")
        results[f'n_gt_{thresh}'] = n_impossible
        results[f'frac_gt_{thresh}'] = frac
    
    # Focus on age_ratio > 0.5 as "impossible"
    impossible = df[df['age_ratio'] > 0.5]
    normal = df[df['age_ratio'] <= 0.5]
    
    logger.info(f"\n'Impossible' galaxies (age_ratio > 0.5): N = {len(impossible)}")
    logger.info(f"'Normal' galaxies (age_ratio ≤ 0.5): N = {len(normal)}")
    
    results['n_impossible'] = len(impossible)
    results['n_normal'] = len(normal)
    
    return results, impossible, normal


def compare_gamma_t(impossible, normal):
    """
    Compare Γ_t between impossible and normal galaxies.
    
    TEP Prediction: Impossible galaxies should have HIGHER Γ_t.
    """
    logger.info("=" * 70)
    logger.info("ANALYSIS 2: Γ_t Comparison (Impossible vs Normal)")
    logger.info("=" * 70)
    
    gamma_impossible = impossible['gamma_t']
    gamma_normal = normal['gamma_t']
    
    mean_impossible = gamma_impossible.mean()
    sem_impossible = gamma_impossible.std() / np.sqrt(len(gamma_impossible))
    
    mean_normal = gamma_normal.mean()
    sem_normal = gamma_normal.std() / np.sqrt(len(gamma_normal))
    
    logger.info(f"Mean Γ_t (impossible): {mean_impossible:.3f} ± {sem_impossible:.3f}")
    logger.info(f"Mean Γ_t (normal): {mean_normal:.3f} ± {sem_normal:.3f}")
    
    # Statistical test
    t_stat, p_value = stats.ttest_ind(gamma_impossible, gamma_normal)
    
    # Effect size (Cohen's d)
    pooled_std = np.sqrt((gamma_impossible.std()**2 + gamma_normal.std()**2) / 2)
    cohens_d = (mean_impossible - mean_normal) / pooled_std
    
    logger.info(f"\nt-statistic: {t_stat:.2f}")
    logger.info(f"p-value: {p_value:.2e}")
    logger.info(f"Cohen's d: {cohens_d:.3f}")
    
    ratio = mean_impossible / mean_normal
    logger.info(f"\nRatio (impossible / normal): {ratio:.2f}")
    
    if mean_impossible > mean_normal and p_value < 0.05:
        logger.info("\n✓ Impossible galaxies have HIGHER Γ_t (TEP-consistent)")
    elif mean_impossible > mean_normal:
        logger.info("\n⚠ Trend in correct direction but not significant")
    else:
        logger.info("\n✗ No Γ_t enhancement in impossible galaxies")
    
    return {
        'mean_gamma_impossible': mean_impossible,
        'sem_gamma_impossible': sem_impossible,
        'mean_gamma_normal': mean_normal,
        'sem_gamma_normal': sem_normal,
        't_statistic': t_stat,
        'p_value': p_value,
        'cohens_d': cohens_d,
        'ratio': ratio,
        'tep_consistent': mean_impossible > mean_normal and p_value < 0.05
    }


def analyze_age_ratio_gamma_correlation(df):
    """
    Test direct correlation between age ratio and Γ_t.
    
    This is the most direct TEP test: higher Γ_t should predict higher age ratio.
    """
    logger.info("=" * 70)
    logger.info("ANALYSIS 3: Age Ratio vs Γ_t Correlation")
    logger.info("=" * 70)
    
    rho, p_value = stats.spearmanr(df['gamma_t'], df['age_ratio'])
    r, p_pearson = stats.pearsonr(df['gamma_t'], df['age_ratio'])
    
    logger.info(f"Spearman ρ = {rho:.3f}, p = {p_value:.2e}")
    logger.info(f"Pearson r = {r:.3f}, p = {p_pearson:.2e}")
    
    # Linear fit
    slope, intercept, r_value, p_fit, std_err = stats.linregress(
        df['gamma_t'], df['age_ratio']
    )
    
    logger.info(f"\nLinear fit: age_ratio = {slope:.3f} × Γ_t + {intercept:.3f}")
    logger.info(f"  Slope: {slope:.3f} ± {std_err:.3f}")
    
    if rho > 0 and p_value < 0.001:
        logger.info(f"\n✓ Strong positive correlation (TEP-consistent)")
    elif rho > 0 and p_value < 0.05:
        logger.info(f"\n✓ Significant positive correlation (TEP-consistent)")
    else:
        logger.info(f"\n⚠ Weak or no correlation")
    
    return {
        'spearman_rho': rho,
        'spearman_p': p_value,
        'pearson_r': r,
        'pearson_p': p_pearson,
        'slope': slope,
        'slope_err': std_err,
        'intercept': intercept,
        'tep_consistent': rho > 0 and p_value < 0.05
    }


def analyze_tep_resolution(df):
    """
    Quantify how much of the "impossibility" TEP explains.
    
    Under TEP, the TRUE age is:
        t_true = t_observed / (1 + Γ_t)
    
    The TEP-corrected age ratio should be < 1 for all galaxies.
    """
    logger.info("=" * 70)
    logger.info("ANALYSIS 4: TEP Resolution of Impossibility")
    logger.info("=" * 70)
    
    # Calculate TEP-corrected age ratio
    df = df.copy()
    df['age_ratio_corrected'] = df['age_ratio'] / (1 + df['gamma_t'])
    
    # How many are still "impossible" after correction?
    n_impossible_raw = (df['age_ratio'] > 0.5).sum()
    n_impossible_corrected = (df['age_ratio_corrected'] > 0.5).sum()
    
    logger.info(f"Impossible galaxies (raw): N = {n_impossible_raw}")
    logger.info(f"Impossible galaxies (TEP-corrected): N = {n_impossible_corrected}")
    
    reduction = (n_impossible_raw - n_impossible_corrected) / n_impossible_raw * 100
    logger.info(f"Reduction: {reduction:.1f}%")
    
    # Mean age ratio before and after
    mean_raw = df['age_ratio'].mean()
    mean_corrected = df['age_ratio_corrected'].mean()
    
    logger.info(f"\nMean age ratio (raw): {mean_raw:.3f}")
    logger.info(f"Mean age ratio (TEP-corrected): {mean_corrected:.3f}")
    
    # For the most extreme cases
    extreme = df[df['age_ratio'] > 0.9]
    if len(extreme) > 0:
        logger.info(f"\nMost extreme cases (age_ratio > 0.9):")
        logger.info(f"  N = {len(extreme)}")
        logger.info(f"  Mean Γ_t = {extreme['gamma_t'].mean():.2f}")
        logger.info(f"  Mean corrected age_ratio = {extreme['age_ratio_corrected'].mean():.3f}")
    
    return {
        'n_impossible_raw': n_impossible_raw,
        'n_impossible_corrected': n_impossible_corrected,
        'reduction_pct': reduction,
        'mean_age_ratio_raw': mean_raw,
        'mean_age_ratio_corrected': mean_corrected
    }


def run_impossible_galaxies_analysis():
    """Run the complete impossible galaxies analysis."""
    logger.info("=" * 70)
    logger.info("TEP-JWST Step 18: 'Impossible' Galaxies Analysis")
    logger.info("=" * 70)
    logger.info("")
    logger.info("Testing TEP prediction: 'impossible' galaxies (age > cosmic age)")
    logger.info("should have the highest predicted Γ_t values.")
    logger.info("")
    
    # Load data
    df = load_uncover_data()
    
    results = {}
    
    # Analysis 1: Identify impossible galaxies
    id_results, impossible, normal = identify_impossible_galaxies(df)
    results['identification'] = id_results
    
    # Analysis 2: Compare Γ_t
    if len(impossible) > 10 and len(normal) > 10:
        results['gamma_comparison'] = compare_gamma_t(impossible, normal)
    
    # Analysis 3: Age ratio vs Γ_t correlation
    results['correlation'] = analyze_age_ratio_gamma_correlation(df)
    
    # Analysis 4: TEP resolution
    results['resolution'] = analyze_tep_resolution(df)
    
    # Summary
    logger.info("")
    logger.info("=" * 70)
    logger.info("SUMMARY")
    logger.info("=" * 70)
    
    corr = results['correlation']
    logger.info(f"Age ratio vs Γ_t: ρ = {corr['spearman_rho']:.3f} (p = {corr['spearman_p']:.2e})")
    
    if 'gamma_comparison' in results:
        comp = results['gamma_comparison']
        logger.info(f"Γ_t ratio (impossible/normal): {comp['ratio']:.2f}")
    
    res = results['resolution']
    logger.info(f"TEP reduces 'impossible' galaxies by: {res['reduction_pct']:.1f}%")
    
    # Overall assessment
    if corr['tep_consistent']:
        logger.info("\n✓ 'Impossible' galaxies are explained by TEP")
        logger.info("  Higher Γ_t → higher apparent age ratio")
    else:
        logger.info("\n⚠ Results inconclusive")
    
    # Save results
    output_file = RESULTS_DIR / "jwst_impossible_galaxies_analysis.json"
    
    def convert_numpy(obj):
        if isinstance(obj, np.ndarray):
            return obj.tolist()
        elif isinstance(obj, (np.integer, np.int64)):
            return int(obj)
        elif isinstance(obj, (np.floating, np.float64)):
            return float(obj)
        elif isinstance(obj, (np.bool_, bool)):
            return bool(obj)
        return obj
    
    results_serializable = json.loads(json.dumps(results, default=convert_numpy))
    
    with open(output_file, 'w') as f:
        json.dump(results_serializable, f, indent=2)
    
    logger.info(f"\nResults saved to: {output_file}")
    
    return results


if __name__ == "__main__":
    run_impossible_galaxies_analysis()
