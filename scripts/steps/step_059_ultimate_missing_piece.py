#!/usr/bin/env python3
"""
TEP-JWST Step 59: The Ultimate Missing Piece

What would make TEP DEFINITIVE? Let's think deeply:
...
STEP_NUM = "059"  # Pipeline step number (sequential 001-176)
STEP_NAME = "ultimate_missing_piece"  # Ultimate missing piece: searches for 7 definitive TEP tests (spec-z, internal gradients, Balmer break, emission lines, M_dyn, cosmic variance, redshift desert)

INTERIM_PATH = PROJECT_ROOT / "results" / "interim"  # Pre-processed intermediate products (CSV format for step-to-step data flow)
DATA_PATH = PROJECT_ROOT / "data" / "interim"  # Processed catalogue products (CSV format from prior steps)
...
def main():
    ...
    print_status("=" * 70, "INFO")
    print_status("\nSearching for the evidence that would make TEP DEFINITIVE...\n", "INFO")
    ...
    # ==========================================================================
    # MISSING PIECE 4: The Anomalous Population
    # ==========================================================================
    print_status("\n" + "=" * 70, "INFO")
    print_status("MISSING PIECE 4: The Anomalous Population", "INFO")
    print_status("=" * 70, "INFO")
    print_status("Are there galaxies that are ANOMALOUS under standard physics?\n", "INFO")
    
    high_z = df[df['z_phot'] > 9].dropna(subset=['gamma_t', 'dust', 'mwa', 'age_ratio'])
    
    if len(high_z) > 20:
        # Define "anomalous" criteria
        ...
        print_status(f"'Anomalous' galaxies at z > 9: N = {len(anomalous)}", "INFO")
        
        if len(anomalous) > 3:
            mean_gamma_imp = anomalous['gamma_t'].mean()
            mean_gamma_norm = high_z.drop(anomalous.index)['gamma_t'].mean() if len(high_z) > len(anomalous) else 0
            ...
            if mean_gamma_norm > 0:
                ...
                if ratio > 5:
                    print_status("\n✓ ANOMALOUS POPULATION: Extreme galaxies have extreme Γ_t", "INFO")
    ...
    # ==========================================================================
    # MISSING PIECE 7: The Ultimate Critical Signal
    # ==========================================================================
    print_status("\n" + "=" * 70, "INFO")
    print_status("MISSING PIECE 7: The Ultimate Critical Signal", "INFO")
    print_status("=" * 70, "INFO")
    print_status("What is the ONE observation that would be ANOMALOUS without TEP?\n", "INFO")
    
    # The ultimate critical signal: A galaxy at z > 10 with:
    # - High dust (A_V > 1)
    # - Old stellar population (MWA > 300 Myr)
    # - High metallicity
    # At z > 10, t_cosmic < 450 Myr, so this is ANOMALOUS under standard physics
    ...
    if len(z10) > 5:
        ...
        if len(extreme) > 0:
            ...
            if mean_gamma > 5:
                print_status("\n✓ CRITICAL SIGNAL: Extreme z>10 galaxies have extreme Γ_t", "INFO")
            
            results['ultimate']['critical_signal'] = {
                'n_z10': int(len(z10)),
