...
def load_data():
    ...
    df = pd.read_csv(INTERIM_DIR / "step_02_uncover_full_sample_tep.csv")
    df = df.rename(
        columns={
            'z_phot': 'z',
            'mwa': 'mwa_Gyr',
...
def analyze_cosmic_age_limit(df):
    ...
    if len(impossible) > 0:
        corrected_ratio = impossible['age_ratio'] / impossible['gamma_t']
...
def analyze_downsizing_transition(df):
    ...
    if len(correlations) >= 2:
        ...
        # Handle edge case where p_trend is NaN (e.g., perfect correlation with only 2 points)
        if np.isnan(p_trend):
