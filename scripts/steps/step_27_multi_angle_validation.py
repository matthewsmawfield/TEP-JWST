...
def load_data():
    ...
    uncover = pd.read_csv(PROJECT_ROOT / "results" / "interim" / "step_02_uncover_full_sample_tep.csv")
...
def residual_test(df):
    ...
    for name in ['impossible', 'extreme']:
...
def run_multi_angle_validation():
    logger.info("=" * 70)
    logger.info("TEP-JWST Step 27: Multi-Angle Validation")
    logger.info("=" * 70)
    logger.info("")
