...
STEP_NUM  = "160"  # Pipeline step number
STEP_NAME = "manuscript_consistency_check"  # Used in log / output filenames
...
JSONS = {
    ...
    "step_176": ROOT / "results/outputs/step_176_nested_bayesian_evidence.json",
}
...
def main() -> None:
    ...
    j176 = json.loads(JSONS["step_176"].read_text())
    ...
    # 2c) Canonical registry should include the nested Bayesian step before final synthesis
    step_176_token = '"step_176_nested_bayesian_evidence.py"'
    has_step_176 = step_176_token in run_all_text
    step_176_before_140 = (
        has_step_176 and has_step_140
        and run_all_text.index(step_176_token) < run_all_text.index(step_140_token)
    )
    checks.append({
        "name": "canonical_registry_includes_nested_bayesian_step_before_final_synthesis",
        "expected": {
            "step_176_present": True,
            "step_140_present": True,
            "step_176_before_step_140": True,
        },
        "found": {
            "step_176_present": has_step_176,
            "step_140_present": has_step_140,
            "step_176_before_step_140": step_176_before_140,
        },
        "pass": has_step_176 and has_step_140 and step_176_before_140,
    })
    ...
    # 5b) step_176 nested Bayesian integration must stay synced to synthesis and manuscript
    nested_bayes = (
        j140.get("final_synthesis", {})
        .get("auxiliary_checks", {})
        .get("nested_bayesian_model_comparison", {})
    )
    ...
    nested_bayes_expected = {
        ...
        "step_140_joint_standard_matches": check_close(
            nested_bayes.get("joint_ln_BF_standard_physics", 0.0),
            j176_joint_bf.get("Standard_Physics", {}).get("ln_BF_TEP_vs_alt", 0.0),
            tol=1e-6,
        ),
        "step_140_joint_agn_matches": check_close(
            nested_bayes.get("joint_ln_BF_agn_feedback", 0.0),
            j176_joint_bf.get("AGN_Feedback", {}).get("ln_BF_TEP_vs_alt", 0.0),
            tol=1e-6,
        ),
        "step_140_residual_null_matches": check_close(
            nested_bayes.get("residual_ln_BF_null", 0.0),
            j176_residual_bf.get("Residual_Null", {}).get("ln_BF_TEP_vs_alt", 0.0),
            tol=1e-6,
        ),
        "step_140_residual_agn_matches": check_close(
            nested_bayes.get("residual_ln_BF_constrained_agn", 0.0),
            j176_residual_bf.get("Constrained_AGN", {}).get("ln_BF_TEP_vs_alt", 0.0),
            tol=1e-6,
        ),
        "results_tokens_present": all(token in texts["results"] for token in results_nested_tokens),
        "discussion_tokens_present": all(token in texts["discussion"] for token in discussion_nested_tokens),
        "generated_root_tokens_present": all(token in generated_root_markdown_text for token in results_nested_tokens[:5]),
    }
    checks.append({
        "name": "nested_bayesian_step176_is_synced_to_synthesis_and_manuscript",
        "expected": {
            "step_140_nested_bayesian_status_live": True,
            "step_140_bayes_factor_fields_match_step176": True,
            "results_and_discussion_tokens_present": True,
            "generated_root_tokens_present": True,
        },
        "found": nested_bayes_expected,
        "pass": all(nested_bayes_expected.values()),
    })
