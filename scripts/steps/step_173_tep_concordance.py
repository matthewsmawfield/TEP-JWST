...
Observables (each fitted independently):
  ...
  5. Impossible galaxy resolution (all surveys, z>7): maximize resolution rate
...
# =============================================================================
# OBSERVABLE 5: Impossible Galaxy Resolution (all surveys z>7)
# =============================================================================

def obs5_impossible_resolution(df_uncover, df_ceers, df_cosmosweb):
    """For each α₀, compute fraction of impossible galaxies resolved."""
    print_status("  Observable 5: Impossible Galaxy Resolution (all surveys z>7)", "INFO")
    ...
    if n_impossible < 5:
        print_status("    Too few impossible galaxies", "WARNING")
        return None
    ...
    return {
        "name": "Impossible Galaxy Resolution (z>7)",
        "short": "Impossible Gal.",
        "alpha0_best": float(best_alpha),
        "alpha0_ci": [float(ci[0]), float(ci[1])],
...
def generate_figure(observables, concordance, output_path):
    ...
    # Cepheid α₀ = 0.58 ± 0.16
    ax1.axvspan(0.58 - 0.16, 0.58 + 0.16, alpha=0.08, color="orange", zorder=0)
    ax1.axvline(0.58, color="orange", linestyle=":", linewidth=1.5, alpha=0.7,
                label="Cepheid α₀")
    ...
    ax2.axvline(0.58, color="orange", linestyle=":", linewidth=1.5, alpha=0.7,
                label="Cepheid α₀")
...
def main():
    ...
    cepheid_tension = abs(concordance["alpha0_weighted_mean"] - 0.58) / max(
        np.sqrt(concordance["alpha0_weighted_sigma"]**2 + 0.16**2), 0.01
    )
    print_status(f"  Tension with Cepheid α₀ = 0.58 ± 0.16: {cepheid_tension:.2f}σ", "INFO")
    concordance["cepheid_tension_sigma"] = float(cepheid_tension)

    # Verdict
    if concordance["concordant"] and cepheid_tension < 2:
        verdict = "PASS: All observables recover consistent α₀, compatible with Cepheid calibration"
    elif concordance["concordant"]:
        verdict = "PARTIAL: Observables are mutually concordant but in tension with Cepheid"
    else:
