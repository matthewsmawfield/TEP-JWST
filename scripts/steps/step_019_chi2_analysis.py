...
STEP_NUM = "019"  # Pipeline step number
STEP_NAME = "chi2_analysis"  # Used in log / output filenames
...
def load_jades_full_catalog():
    """Download (if needed) and load the full JADES GOODS-S Deep catalog.

    The GOODS-S Deep photometry v2.0 catalog (Rieke et al. 2023;
    Robertson et al. 2023) contains ~45k sources with:
      - FLAG extension: IDs, positions, quality flags
      - SIZE extension: Kron radii, detection pixel counts
      - PHOTOZ extension: EAZY photo-z posteriors and chi-squared
      - KRON extension: aperture photometry in all NIRCam bands

    High-z candidates (4 < z_phot < 15) are selected. Stellar masses
    are roughly estimated from F277W flux; Gamma_t is computed from
...
def load_jades_z8_candidates():
    """Load the JADES z > 8 candidate catalog (Hainline et al. 2023).

    This smaller catalog focuses on spectroscopically or photometrically
    confirmed z > 8 candidates. MUV is derived from F150W (which probes
    rest-frame UV at z ~ 8-12). Gamma_t is estimated from a rough
    MUV-to-halo-mass conversion.
    """
...
def analyze_photoz_uncertainty(df):
    """TEST 1: Photo-z uncertainty scaling with Gamma_t.

    EAZY photo-z posteriors are based on template fitting that assumes
    standard stellar evolution. TEP-induced SED distortions (older-
    looking spectra at fixed cosmic time) cause template mismatch,
    broadening the posterior and inflating z_phot_err = (u68 - l68)/2.

    Prediction: positive Spearman rho(Gamma_t, z_phot_err).
    """
    logger.info("=" * 70)
...
def analyze_spec_phot_comparison(df):
    """TEST 2: Spectroscopic vs photometric redshift comparison.

    Spectroscopic redshifts from emission lines are immune to TEP
    (atomic transitions are unaffected). Photometric redshifts from
    broadband SED fitting are susceptible to TEP-induced aging.

    Both the absolute offset |z_phot - z_spec| and the signed offset
    are tested for correlation with Gamma_t. Additionally, the mean
    Gamma_t of spec-z vs phot-only subsamples is compared.
    """
    logger.info("=" * 70)
...
def analyze_chi2_scaling(df):
    """TEST 3: SED chi-squared scaling with Gamma_t.

    The EAZY chi-squared of the best-fit template quantifies
    how well the standard (isochronous) SED library matches the
    observed photometry. Under TEP, galaxies with high Gamma_t
    have SEDs that deviate systematically from isochronous models,
    producing higher chi-squared values.

    Binned analysis provides a visual diagnostic: the mean and median
    chi-squared should increase monotonically with Gamma_t.
    """
    logger.info("=" * 70)
...
def analyze_size_mass_relation(df):
    """TEST 4: Size-mass relation anomaly (the compactness paradox).

    High-z massive galaxies are observed to be surprisingly compact
    compared to local mass-size relations. Under TEP, the explanation
    is straightforward: SED fitting overestimates M* for high-Gamma_t
    galaxies, so at fixed *apparent* mass they are smaller than
    expected.

    Method:
      1. Fit log(r_Kron) = a * log(M*) + b to define the mean
         mass-size relation.
      2. Compute size residuals: Delta_log_r = log(r) - predicted.
      3. Test for negative correlation between Delta_log_r and Gamma_t.

    A significant negative rho confirms the TEP compactness prediction.
    """
    logger.info("=" * 70)
...
def analyze_delta_chi2_diagnostic(df):
    """TEST 5: Delta-chi-squared as a TEP diagnostic.

    Delta_chi2 = chi2(z < 7 solution) - chi2(z_best) measures the
    preference for the high-z solution over a low-z interloper.
    Under TEP, high-Gamma_t galaxies have SEDs that are more
    distinctly "high-z looking" because the enhanced aging shifts
    the SED further from any plausible low-z template.

    Prediction: positive Spearman rho(Gamma_t, Delta_chi2), meaning
    stronger rejection of the low-z solution for TEP-enhanced systems.
    """
    logger.info("=" * 70)
