...
STEP_NUM = "012"  # Pipeline step number
STEP_NAME = "sn_ia_extended"  # Used in log / output filenames
...
def load_pantheon_data():
    """Load Pantheon+SH0ES data from the TEP-H0 repository.

    The Pantheon+ dataset (Scolnic et al. 2022) contains 1701 light curves
    of 1550 spectroscopically confirmed SN Ia from 18 surveys, spanning
    z = 0.001 to 2.26. Each SN has standardized corrected magnitude
    (m_b_corr), light-curve shape (x1), colour (c), host galaxy mass
    (HOST_LOGMASS), and a calibrator flag (IS_CALIBRATOR).
    """
...
def analyze_mass_step_redshift_evolution(df):
    """Test if the SN Ia mass step evolves with redshift.
    ...
    Method:
      ...
      in 6 redshift bins from z = 0.01 to 1.0, then correlate the
      step amplitude with redshift via Spearman rank.
    """
    logger.info("=" * 70)
...
def analyze_calibrator_offset(df):
    ...
    TEP prediction:
      SH0ES calibrators are anchored in group/cluster environments
      (LMC in the Local Group, NGC 4258, M31) where the TEP scalar
      field is screened by the ambient group potential. Hubble flow
      SNe are in mixed (field + group) environments with less screening.
      This creates a systematic offset in mean Hubble residuals:
        offset = mean(calibrators) - mean(Hubble flow)
      which contributes directly to the Hubble tension.
    """
    logger.info("=" * 70)
...
def analyze_residual_correlations(df):
    """Test for correlations between Hubble residuals and host properties.

    Beyond the binary mass step, TEP predicts continuous correlations
    between Hubble residuals and any host property that traces the
    gravitational potential depth: stellar mass, morphological type,
    star formation rate, and local environment density.

    SN-intrinsic properties (colour c, stretch x1) should not correlate
    with residuals after standardisation unless TEP introduces a systematic
    environmental dependence into the standardisation itself.
    """
    logger.info("=" * 70)
...
def analyze_high_z_mass_step(df):
    """Focus on high-z (z > 0.1) SNe where TEP effects should be strongest.

    At z > 0.1, galaxies are generally more compact and denser than their
    local counterparts. Under TEP, this increases sigma and hence the
    TEP bias for high-mass hosts, amplifying the mass step relative to
    the full (mostly low-z) sample. We compare the high-z mass step to
    the full-sample mass step and report the ratio.
    """
    logger.info("=" * 70)
