#!/usr/bin/env python3
"""
TEP-JWST Step 173: Cosmic Star Formation Rate Density (SFRD) Correction
...
Output:
  - results/outputs/step_173_cosmic_sfrd_correction.json
  - results/figures/figure_173_cosmic_sfrd_correction.png
...
STEP_NUM = "173"  # Pipeline step number
STEP_NAME = "cosmic_sfrd_correction"  # Used in log / output filenames
...
def comoving_volume_shell(z_lo, z_hi, area_arcmin2, H0=67.4, Om=0.315):
    ...
    # Comoving distance at z_lo and z_hi
    dc_lo, _ = integrate.quad(dc_integrand, 0, z_lo)
    dc_hi, _ = integrate.quad(dc_integrand, 0, z_hi)
    dc_lo *= DH
    dc_hi *= DH
    ...
    # Shell volume
    vol = (area_sr / 3.0) * (dc_hi**3 - dc_lo**3)
    return vol
...
def main():
    ...
    print_status("Step 173: Cosmic SFRD Correction at z > 6", "INFO")
    print_status("=" * 65, "INFO")
