...
"""
TEP-JWST Step 8: Seven Threads Summary
...
def load_thread_results():
    ...
    thread_files = [
        ...
        ("thread_3", "step_005_thread2_4_partial_correlations.json", "thread3_gamma_metallicity"),
        ("thread_4", "step_005_thread2_4_partial_correlations.json", "thread4_gamma_dust"),
        ("thread_5", "step_006_thread5_z8_dust.json",                None),
        ("thread_6", "step_007_thread6_7_coherence.json",            "thread6_age_metallicity"),
        ("thread_7", "step_007_thread6_7_coherence.json",            "thread7_multi_property"),
    ]
    
    for key, filename, sub_key in thread_files:
        filepath = OUTPUT_PATH / filename
        if filepath.exists():
            with open(filepath) as f:
                data = json.load(f)
            threads[key] = data[sub_key] if sub_key and sub_key in data else data
        else:
...
def main():
    ...
    print_status(f"Saved: {OUTPUT_PATH / f'step_{STEP_NUM}_summary.json'}", "INFO")
