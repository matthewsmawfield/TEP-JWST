...
def main():
    ...
    if len(enhanced) > 30 and len(suppressed) > 30:
        ...
        if abs(rho_enh) < abs(rho_sup):
            print_status(f"\n✓ SFR-Age coupling is weaker in enhanced regime (ρ_enh = {rho_enh:.3f} vs ρ_sup = {rho_sup:.3f})", "INFO")
    ...
    if rho_resid > 0 and p_resid < 0.001:
        print_status(f"\n✓ High-Γ_t galaxies are older than expected at fixed sSFR (ρ = {rho_resid:.3f}, p = {p_resid:.2e})", "INFO")
        age_resid_positive = True
    elif rho_resid > 0 and p_resid < 0.05:
        print_status(f"\n⚠ Moderate evidence for age inflation at fixed sSFR (ρ = {rho_resid:.3f}, p = {p_resid:.2e})", "INFO")
        age_resid_positive = True
    else:
    ...
    if n_tests_passed >= 2:
        print_status(f"\n✓ SFR-age decoupling detected ({n_tests_passed}/3 tests passed)", "INFO")
        print_status("  High-Γ_t galaxies appear older than their SFR would suggest.", "INFO")
        overall = "detected"
    elif n_tests_passed == 1:
        print_status(f"\n⚠ Partial evidence for SFR-age decoupling ({n_tests_passed}/3 tests passed)", "INFO")
        overall = "partial"
    else:
