...
from sklearn.decomposition import PCA  # Principal component analysis for coherence test
from sklearn.preprocessing import StandardScaler  # Feature normalisation before PCA
...
STEP_NUM = "051"  # Pipeline step number (sequential 001-176)
STEP_NAME = "temporal_coherence"  # Temporal coherence: deepest TEP test checking if 5 independent age indicators (MWA, dust, SFH, metallicity, chi²) align under Gamma_t correction
...
def main():
    ...
    if len(high_z) > 50:
        ...
        if rho_eff > rho_cosmic + 0.1:
            print_status(f"✓ t_eff predicts dust better than t_cosmic (Δρ = {rho_eff - rho_cosmic:.3f})", "INFO")
    ...
    if len(high_z) > 50:
        ...
        if std_corrected < std_apparent * 0.8:
            print_status(f"✓ TEP correction reduces age_ratio scatter ({(1 - std_corrected/std_apparent)*100:.1f}% reduction)", "INFO")
