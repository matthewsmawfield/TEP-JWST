...
def main():
    ...
    # ==========================================================================
    print_status("\n" + "=" * 70, "INFO")
    print_status("Functional Form Discrimination Test: t_eff vs t_cosmic", "INFO")
    print_status("=" * 70, "INFO")
    ...
    if len(high_z) > 50:
        ...
        if wins_teff >= 2 and total_improvement > 0.3:
            print_status("\n" + "=" * 70, "INFO")
            print_status(f"t_eff outperforms t_cosmic ({wins_teff}/3 tests, Δρ = {total_improvement:+.4f})", "INFO")
            print_status("=" * 70, "INFO")
            print_status("The functional form of TEP (t_eff) outperforms", "INFO")
            print_status("the standard model (t_cosmic) across multiple independent observables.", "INFO")
            verdict = "supported"
        else:
            print_status(f"\n⚠ Mixed results ({wins_teff}/3 wins for t_eff, Δρ = {total_improvement:+.4f})", "INFO")
        ...
        # ==========================================================================
        # Critical signal: Δρ for dust prediction
        # ==========================================================================
        print_status("\n" + "=" * 70, "INFO")
        print_status("Critical signal: Δρ for dust prediction", "INFO")
        print_status("=" * 70, "INFO")
        ...
        print_status(f"\nInterpretation:", "INFO")
        print_status(f"  • t_cosmic alone: ρ = {rho_cosmic_dust:+.4f}", "INFO")
        print_status(f"  • t_eff (TEP): ρ = {rho_eff_dust:+.4f}", "INFO")
        print_status(f"  • Improvement: {delta_rho_dust:+.4f}", "INFO")
        print_status(f"\nThis means:", "INFO")
        print_status(f"  • Cosmic time has limited predictive power for dust at z > 8", "INFO")
        print_status(f"  • TEP-corrected time has stronger predictive power", "INFO")
