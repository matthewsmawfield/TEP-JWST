...
STEP_NAME = "screening_scale"  # Screening scale: derives scalar field Compton wavelength λ_C ~ 1/m_eff from chameleon theory with V(φ) = Λ^4(1 + Λ^n/φ^n)
...
def derive_compton_wavelength(M_pl=2.435e18, beta=0.58, Lambda=2.3e-3):
    ...
    # In chameleon theory, the screening scale is the "thin-shell" radius
    # r_thin ~ (β * M_pl / ρ * Δφ)^(1/3) for compact objects
...
def main():
    ...
    print(f"  Expected range: {pred['expected_range_kpc'][0]:.1f}-{pred['expected_range_kpc'][1]:.1f} kpc")
    print(f"  Within observed range: {pred['within_observed_range']}")
    
    print(f"\nObserved vs Predicted:")
    print(f"  Observed core scale: {results['observed_scale']['core_radius_kpc']:.1f} kpc")
