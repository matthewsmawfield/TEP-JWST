...
from scripts.utils.tep_model import (  # TEP model: alpha_0=0.58, screening density rho_c=20 g/cm³
    ALPHA_0,
    RHO_CRIT_G_CM3,
)
...
def temporal_topology_screening_profile(
    rho, rho_c=RHO_CRIT_G_CM3, beta=ALPHA_0, transition_width=0.5
):
    """
...
def temporal_shear_suppression(
    rho, rho_c=RHO_CRIT_G_CM3, alpha_0=ALPHA_0, transition_width=0.5
):
    """
    ...
    alpha_eff : float or array
        Effective coupling after suppression (alpha_eff << alpha_0 in dense regimes)
    """
...
def compute_effective_coupling(
    rho, rho_c=RHO_CRIT_G_CM3, alpha_0=ALPHA_0, model="chameleon"
):
    """
    ...
    if model == "temporal_topology":
        ...
        alpha_eff = temporal_shear_suppression(rho, rho_c, alpha_0)
...
def run_analysis():
    ...
    idx_90 = np.argmin(np.abs(tep_profile - 0.9 * alpha_0))
    idx_10 = np.argmin(np.abs(tep_profile - 0.1 * alpha_0))
    ...
    for name, ref_rho in reference_densities.items():
        alpha_eff = compute_effective_coupling(ref_rho, rho_c, alpha_0, "temporal_topology")
