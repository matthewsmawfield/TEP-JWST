...
def main():
    ...
    # Color map by domain
    domain_colors = {
    'High-z Dust': '#1f77b4',      # Blue
    'sSFR Inversion': '#ff7f0e',   # Orange
    'Screening': '#2ca02c',         # Green
    'SN Ia': '#d62728',             # Red
    'Distance Ladder': '#9467bd',   # Purple
    'Regime Properties': '#8c564b', # Brown
    'LRD/BH': '#e377c2',            # Pink
    'Spectroscopic': '#7f7f7f',     # Gray
    }
    ...
    # For the main correlation plot, use normalized values
    # But we want to show actual values, so we need separate handling
    
    # 1:1 line
    ax.plot([val_min, val_max], [val_min, val_max], 'k--', linewidth=2, 
           label='Perfect Agreement', alpha=0.7)
    
    # Shaded region for ±20% agreement
    ...
    legend_elements = [Patch(facecolor=color, edgecolor='black', label=domain)
                     for domain, color in domain_colors.items() if domain in [p['domain'] for p in predictions_data]]
    ax.legend(handles=legend_elements, loc='upper left', fontsize=10)
    
    # Add statistics text box
    textstr = f'N = {n_points}\n'
    textstr += f'Within 2σ: {n_agree}/{n_points}\n'
    textstr += f'χ²/dof = {reduced_chi2:.2f}\n'
    ...
    # Also create a multi-panel version grouped by domain
    try:
        set_pub_style()
    except NameError:
        pass
    ...
    for i, domain in enumerate(domains[:8]):
        ax = axes2[i]
        domain_data = [p for p in predictions_data if p['domain'] == domain]
        ...
        # 1:1 line
        all_d = [p['tep_prediction'] for p in domain_data] + [p['observation'] for p in domain_data]
    ...
    plt.suptitle('TEP Predictions vs Observations by Domain', fontsize=14, fontweight='bold')
    plt.tight_layout()
