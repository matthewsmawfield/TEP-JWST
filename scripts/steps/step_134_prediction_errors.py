...
TEP prediction error budget — kappa_gal uncertainty propagation
...
STEP_NUM  = "134"  # Pipeline step number
STEP_NAME = "prediction_errors"  # Used in log / output filenames
...
import numpy as np
from scripts.utils.tep_model import compute_gamma_t, stellar_to_halo_mass_behroozi_like, KAPPA_GAL, KAPPA_GAL_UNCERTAINTY  # Shared TEP model
...
def run():
    ...
    for m in FIDUCIAL_LOG_MSTAR:
        for z in FIDUCIAL_Z:
            ...
            delta_g = abs(dg_da) * KAPPA_GAL_UNCERTAINTY
    ...
    if cw_file.exists():
        ...
        if "dust" in df.columns and len(df) > 10:
            ...
            for a in ALPHA_GRID:
                gt = compute_gamma_t(log_mh, df_valid["z_phot"].values, kappa=a)
    ...
    if rho_vs_alpha:
        ...
        # Finite difference around kappa_gal
        idx0 = np.argmin(np.abs(np.array(alphas) - KAPPA_GAL))
        drho_da = float(
            np.gradient(np.array(rhos), np.array(alphas))[idx0]
        )
        delta_rho_1sigma = abs(drho_da) * KAPPA_GAL_UNCERTAINTY
        rho_at_kappa_gal = rhos[idx0]
        logger.info(
            f"  d(rho)/d(kappa_gal) = {drho_da:.3f}"
            f"  delta_rho(1-sigma) = {delta_rho_1sigma:.4f}"
        )
    else:
        drho_da = float("nan")
        delta_rho_1sigma = float("nan")
        rho_at_kappa_gal = float("nan")
        rho_vs_alpha = []
    ...
    result = {
        ...
        "name":   STEP_NAME,
        "status": "SUCCESS",
        "description": "TEP prediction error budget — kappa_gal uncertainty propagation",
        ...
        "rho_at_kappa_gal":       rho_at_kappa_gal,
        "conclusion": (
            f"1-sigma kappa_gal uncertainty (+/-{KAPPA_GAL_UNCERTAINTY}) "
