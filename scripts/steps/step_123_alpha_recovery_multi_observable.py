...
def compute_observable_correlation(observable, log_mh, z, alpha_0):
    ...
    absolute scale of log_Gamma_t, which changes with alpha_0."""
...
def fit_alpha_single_observable(observable, log_mh, z, obs_name, expected_sign=1):
    ...
    sensitive to the absolute variance of log_Gamma_t, which scales with alpha_0.
...
def main():
    ...
    # Load data
    possible_paths = [
        INTERIM_DIR / "step_02_uncover_full_sample_tep.csv",
        INTERIM_DIR / "step_02_uncover_multi_property_sample_tep.csv",
    ]
    ...
    # Save results
    output_path = OUTPUTS_DIR / f"step_{STEP_NUM}_alpha_recovery.json"
