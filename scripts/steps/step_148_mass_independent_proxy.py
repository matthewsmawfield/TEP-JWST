...
- results/figures/figure_176_mass_independent_proxy.png
"""
...
def partial_spearman(x, y, covariates):
    ...
    x_resid, mask_x = residualize(x, covariates)
    y_resid, mask_y = residualize(y, covariates)
    mask = mask_x & mask_y & np.isfinite(x_resid) & np.isfinite(y_resid)
    if mask.sum() < 10:
        return 0.0, 1.0, 0
    rho, p = stats.spearmanr(x_resid[mask], y_resid[mask])
...
def main():
    ...
    # sSFR residual after removing mass trend
    covariates_mz = df[["log_mstar", "z"]].values
    df["ssfr_resid"], mask_ssfr = residualize(df["log_ssfr_col"].values, covariates_mz)
    
    # Chi2 as SED tension proxy
    df["log_chi2"] = np.log10(df["chi2"].clip(lower=0.01))
    
    # Age ratio residual
    df["age_ratio_resid"], _ = residualize(df["age_ratio"].values, covariates_mz)
    ...
    for sample_name, sample_mask in subsamples.items():
        ...
        sample_results["test1_sfr_surface_density"] = {
            ...
            "mass_independent_signal": p_part < 0.05,
            "survives_gamma_t_control": p_after_gt < 0.05,
        }
        ...
        # ── Combined Assessment ─────────────────────────────────────────
        n_pass = sum([
            sample_results["test1_sfr_surface_density"]["mass_independent_signal"],
            float(p_ssfr) < 0.05,
            float(p_chi2) < 0.05,
            float(p_zunc) < 0.05,
            float(p_age) < 0.05,
        ...
        sample_results["summary"] = {
            "tests_significant_at_005": int(n_pass),
            "tests_total": 5,
            "fraction_significant": n_pass / 5,
            "assessment": (
                f"{n_pass}/5 mass-independent proxies significantly predict dust "
    ...
    results["headline"] = {
        "z_gt_8_tests_passing": z8_summary.get("tests_significant_at_005", 0),
        "z_gt_8_tests_total": 5,
        "conclusion": (
            "Mass-independent proxy tests provide partial support for the TEP "
            "hypothesis. The tests use SFR surface density, sSFR residuals, "
            "SED chi², photo-z uncertainty, and age-ratio residuals as proxies "
            "for potential depth that are independent of stellar mass. "
            "These tests do not definitively break the mass-proxy degeneracy — "
            "that requires IFU velocity dispersions — but they provide evidence "
            "that the dust signal at z>8 is not purely a mass artefact."
        ),
        "honest_caveat": (
    ...
    for sample_name, sample_data in results["tests"].items():
        if "summary" in sample_data:
            ...
            print_status(f"  {sample_name}: {s['tests_significant_at_005']}/5 tests pass")
...
def make_figure(results, df, z8_mask):
    ...
    summary_lines.append(f"\n{s.get('tests_significant_at_005', 0)}/5 tests pass at p<0.05")
